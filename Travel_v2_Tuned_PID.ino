...
/*--------------Variables for Computing PID--------------*/
  ...
  const double kp_L = 13, ki_L = 0.125, kd_L = 0.52;//proportional, integral, derivative gains
  const double kp_R = 40, ki_R = 0.2, kd_R = 0.00001;  
...
//  Serial.println(SetPointLeft-AccErr);
  ...
  while(SetPointLeft != PulseCountLeft){//Exit loop if Pulse Count falls with in a range around the Setpoint
